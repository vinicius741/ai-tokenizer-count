...
<task type="auto">
  ...
  <verify>grep -E "class JobQueue|processNext|enqueue|getStatus" server/src/lib/job-queue.ts</verify>
  <done>JobQueue class is exported with enqueue, processNext, getStatus, and cancel methods</done>
</task>
...
3. Verify methods exist: grep -E "processNext|enqueue|getStatus" server/src/lib/job-queue.ts
4. Verify types are exported: grep -E "JobStatus|JobState|ProcessRequest" packages/shared/src/types.ts
5. Verify shared package builds: npm run build --workspace=@epub-counter/shared
</verification>

<success_criteria>
- JobQueue class exists with enqueue, processNext, getStatus, cancel methods
