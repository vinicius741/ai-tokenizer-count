...
<task type="auto">
  <name>Task 1: Add TokenizerInfo type to shared package</name>
  <files>packages/shared/src/types.ts</files>
  <action>
    Add TokenizerInfo type to packages/shared/src/types.ts:
    ...
    Note: This type may already exist - verify before adding.
  </action>
  <verify>grep -q "interface TokenizerInfo" packages/shared/src/types.ts && echo "TokenizerInfo type exists"</verify>
  <done>TokenizerInfo type is exported from @epub-counter/shared</done>
</task>
