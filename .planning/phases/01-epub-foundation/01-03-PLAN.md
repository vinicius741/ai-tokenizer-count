...
must_haves:
  ...
  artifacts:
    - path: "src/cli/index.ts"
      ...
      contains: "Command() setup, argument parsing, action handler"
...
<objective>
Build the CLI interface using commander for argument parsing and cli-table3 for formatted table output display.

Purpose: Provide users with a command-line interface that accepts EPUB files or folders, processes them, and displays results in a readable table format.

Output: Working CLI with argument parsing, help text, and table output display (no error handling yet — that's Plan 04).
</objective>
...
<task type="auto">
  <name>Task 1: Implement CLI interface with commander</name>
...
3. Setup Commander program:
   ...
   - action((paths, options) => { handle input vs --input precedence, call processEpubs })
...
DO NOT implement error handling in this task — errors will crash the process (Plan 04 adds continue-on-error).
  ...
  <verify>Run `npm run build && node dist/cli/index.js --help` — should display help text with all options and arguments</verify>
  <done>CLI accepts positional paths and --input flag, shows help text, defaults to ./epubs/, supports --verbose and --recursive flags</done>
</task>
...
2. `node dist/cli/index.js --help` displays help text
3. CLI accepts all documented arguments and flags
4. Table displays with proper formatting when run with EPUB files
</verification>

<success_criteria>
1. CLI executable via `node dist/cli/index.js`
2. Help text displays all options (--input, --verbose, --recursive)
