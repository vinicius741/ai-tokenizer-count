...
<task type="auto">
  ...
  <files>package.json, tsconfig.json, src/cli/index.ts, src/epub/parser.ts, src/epub/text.ts, src/output/table.ts, src/output/markdown.ts, src/output/json.ts, src/errors/handler.ts</files>
  <action>
...
- src/output/json.ts (empty export)
...
<task type="auto">
  <name>Task 3: Update .gitignore for epubs and results folders</name>
  <files>.gitignore</files>
  <action>
Append to existing .gitignore:
- Line: "./epubs/" (default input folder per CFG-01, CFG-03)
- Line: "./results/" (default output folder per CFG-02)
...
Note: Use ./results/ per CFG-02 requirement (not ./output/).
  </action>
  <verify>Run `git status` — should not show node_modules/, dist/, epubs/, or results/ as untracked if they exist</verify>
  <done>.gitignore includes ./epubs/, ./results/, dist/, and node_modules/ patterns</done>
</task>
...
6. .gitignore prevents epubs/, results/, dist/, and node_modules/ from being committed
</success_criteria>
