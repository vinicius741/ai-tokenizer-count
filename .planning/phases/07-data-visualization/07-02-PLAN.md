...
must_haves:
  truths:
    ...
    - "User can zoom and pan scatter plot when viewing 50+ EPUBs"
  artifacts:
...
<task type="auto">
  <name>Add zoom and pan to ScatterChart</name>
  <files>web/src/components/visualization/ScatterChart.tsx</files>
  <action>
    Update web/src/components/visualization/ScatterChart.tsx to add zoom/pan functionality:

    1. Import Brush component from recharts: import { Brush } from 'recharts'
    2. Add Brush component inside ScatterChart after Legend:
       - dataKey="x" (brush controls x-axis/word count range)
       - height={30} (compact brush at bottom)
       - stroke="#8884d8" (matching chart theme)
       - fill="#e0e7ff" (light indigo fill for selected area)
    3. Brush enables users to drag to select a word count range to zoom into
    4. Double-click brush area to reset zoom to full range

    This is essential for viewing 50+ EPUBs where points become densely packed.

    CRITICAL: Recharts Brush works automatically with ScatterChart - no additional state management needed.
  </action>
  <verify>
    ```bash
      grep -q "import.*Brush.*from 'recharts'" web/src/components/visualization/ScatterChart.tsx
    ```
  </verify>
  <done>
    ScatterChart.tsx includes Brush component for zoom/pan on word count axis
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
    ...
    - Brush component for zoom/pan functionality
  </what-built>
  <how-to-verify>
    ...
    9. Drag on the brush area at bottom of chart - verify zoom works
    10. Double-click brush area - verify zoom resets to full range
  </how-to-verify>
...
11. Brush component enables zoom/pan on word count axis
12. Double-clicking brush resets zoom to full range
</verification>
...
8. Brush area at bottom enables zooming by dragging to select range
9. Double-clicking brush area resets zoom to show all data points
</success_criteria>
