...
must_haves:
  truths:
    ...
    - "Parent component receives selected tokenizer changes via onSelectionChange callback"
  artifacts:
  ...
  key_links:
    ...
    - from: "web/src/App.tsx"
      ...
      via: "onSelectionChange prop callback"
...
- Accept onSelectionChange prop to notify parent of selection changes
- Call onSelectionChange callback whenever selectedTokenizers changes
- Show validation error if no tokenizers selected
- Display selected HF models as removable badges
- Use Sonner for error toasts
  </action>
  <verify>web/src/components/tokenizer/TokenizerSelector.tsx exists and exports TokenizerSelector component with onSelectionChange prop</verify>
  <done>TokenizerSelector created with ToggleGroup, HFModelCombobox integration, localStorage persistence, validation, and parent callback</done>
</task>
...
8. TokenizerSelector calls onSelectionChange callback when selection changes
9. Parent component receives selected tokenizers via callback
</success_criteria>
