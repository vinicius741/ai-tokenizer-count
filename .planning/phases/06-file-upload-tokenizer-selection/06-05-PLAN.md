...
- Call onCancel callback when Cancel clicked
- Show info toast after cancel
  </action>
  <verify>web/src/components/processing/ProcessButton.tsx updated with Cancel button and isProcessing prop</verify>
  <done>ProcessButton now shows Cancel button during processing and Process button when idle</done>
</task>
...
3. Cancelled state message appears after cancel
4. Reset button appears after cancel/completion
5. Reset button clears all state except tokenizers
6. Tokenizer selections persist across page refreshes
7. localStorage key 'selected-tokenizers' contains saved selections
8. UI properly manages all states (idle, processing, cancelled, complete)
9. SSE connections properly abort on cancel
10. Full workflow (process -> cancel -> reset -> process) works smoothly
</success_criteria>
