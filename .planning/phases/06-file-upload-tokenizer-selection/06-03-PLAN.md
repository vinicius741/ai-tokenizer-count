...
must_haves:
  truths:
    ...
    - "Process button receives selected tokenizers from TokenizerSelector via callback"
...
- POST /api/process — Accepts { inputPath: string, tokenizerList: string[] }, returns { jobId: string }
...
<task type="auto">
  ...
  <action>
Update App.tsx to integrate TokenizerSelector, FileDropzone, FolderInput, and ProcessButton with proper state wiring:
...
- Wire TokenizerSelector onSelectionChange to handleTokenizerSelectionChange callback
- Update selectedTokenizers state when tokenizer selection changes
- Pass selectedTokenizers to ProcessButton for enable/disable logic
- Show file upload OR folder input options
- Show process button at bottom
- Display current job ID for debugging
  </action>
  <verify>web/src/App.tsx updated with TokenizerSelector (with onSelectionChange prop), FileDropzone, FolderInput, and ProcessButton components</verify>
  <done>App.tsx integrates all components with proper state wiring between TokenizerSelector and ProcessButton</done>
</task>
...
6. TokenizerSelector selection changes update App.tsx selectedTokenizers state
7. ProcessButton receives selected tokenizers from parent state
8. Clicking Process calls POST /api/process with correct data
9. Loading state shown during API call
10. Success/error messages shown as toasts
11. JobId is returned and displayed
</success_criteria>
